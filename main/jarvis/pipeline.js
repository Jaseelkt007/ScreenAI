...
/**
 * Run the full pipeline from a pre-transcribed string (M2 — no STT step).
 *
...
async function runPipelineFromText(transcript, hudSend, waitForConfirm) {
  ...
  try {
    ...
    const t1 = Date.now();
    const classifierResult = await classify(transcript);
    ...
    const t3 = Date.now();
    const verifierResult = await verify(classifierResult, toolResult);
...
async function speakAndDone(hudSend, ok, display, spokenText, timings, t0) {
  // Attempt TTS even on errors (e.g. "I don't know how to do that")
  let audioBase64 = null;
  let mimeType    = null;
...
function buildConfirmMessage(classifierResult) {
  ...
  if (intent === 'file.write') {
    return `This will overwrite existing content in "${params.name}". Continue?`;
  }
  if (intent === 'file.append') {
    return `Append ${params.content?.length || 0} characters to "${params.name}"?`;
  }
